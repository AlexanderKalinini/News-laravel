    ...
    <div class="w-96 rounded bg-white p-5 shadow-xl">
      ...
      <form class="mt-5 space-y-5" method="POST" action={{ route('sendContactForm') }}>
        ...
        <input name="email" value="{{ old('email') }}" type="text"
          class="h-12 w-full rounded border border border-gray-800 border-red-500 px-3" placeholder="Email" />
        @error('email')
          <p class="text-red-500">{{ $message }}</p>
        @enderror

        <input name="text" value="{{ old('text') }}" type="text"
          class="h-12 w-full rounded border border-gray-800 px-3" placeholder="Сообщение" />
        @error('text')
          <p class="text-red-500">{{ $message }}</p>
        @enderror
        <button type="submit"
